 ...
 */
package yszhh.wlgj.com.yunshangzhihui_android.widgets;
...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    public static boolean onTouch = false;// 鏄惁鐐瑰嚮鍒颁簡viewFlow鐨勫尯鍩燂紝鐢ㄤ簬鑷畾涔夊浘灞備腑閲嶅畾鍚戜簨浠剁殑浼犻�掋��
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN:
             ...
             * will be false if being flinged.
             */
            onTouch = true; // 鐐瑰嚮鍒皏iewflow鐨勫尯鍩熶腑
            if (!mScroller.isFinished()) {
                mScroller.abortAnimation();
            }
        ...
        case MotionEvent.ACTION_UP:
            onTouch = false;// 鎵嬫寚鎶捣锛屽皢鏁板�艰繘琛岄噸缃�
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                final VelocityTracker velocityTracker = mVelocityTracker;
                velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
                int velocityX = (int) velocityTracker.getXVelocity();
    ...
    private void resetFocus() {
        ...
        for (int i = Math.max(0, mCurrentAdapterIndex - mSideBuffer); i < Math.min(mAdapter.getCount(), mCurrentAdapterIndex + mSideBuffer
                + 1); i++) {
